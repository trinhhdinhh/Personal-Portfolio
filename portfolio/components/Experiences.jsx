...
const Experiences = () => {
  ...
  return (
    <div id="experiences"className='w-full px-[12%] py-10 scroll-mt-20'>
      <h4 className='text-center mb-2 text-lg font-ovo dark:text-gray-300'>My Journey</h4>
      <h2 className='text-center text-5xl font-ovo dark:text-white'>Experiences</h2>

      <div className='relative px-6 max-w-4xl mx-auto mt-16'>

        {/* Timeline */}
        <div className='absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-linear-to-b bg-black dark:bg-gray-600 md:-translate-x-px'> </div>
        {experiences.map((item, index)=>{
          return <div key ={index} className={`relative flex flex-col md:flex-row gap-8 mb-12 ${index %2 === 0? "md:flex-row-reverse": ""}`}>

          {/* Timeline Node */}
          <div className='absolute left-4 md:left-1/2 w-8 h-8 -translate-x-1/2 rounded-full bg-white dark:bg-gray-800 border-2 border-black dark:border-gray-600 flex items-center justify-center z-10'>
            {item.type === "work" ? (
              <BsBriefcase className='text-black dark:text-white text-sm' />
            ) : (
              <PiGraduationCap className='text-black dark:text-white text-base' />
            )}
          </div>
          ...
          <div className='ml-16 md:ml-0 md:w-[calc(50%-2rem)]'>
            <div className='bg-white dark:bg-gray-800 shadow-md rounded-xl p-6 hover:scale-105 transition-all duration-300'>
              ...
              <h3 className='text-xl font-ovo font-semibold mb-1 dark:text-white'>{item.title}</h3>
              <p className='text-muted-foreground dark:text-gray-400 text-sm mb-3 font-ovo'>{item.company}</p>
              <p className='text-muted-foreground dark:text-gray-300 text-sm mb-4 font-ovo'>{item.description}</p>
              <div className='flex flex-wrap gap-2'>
                {item.skills.map((skills)=>{
                  return <span key={skills} className='text-xs px-2 py-1 rounded-md bg-purple-600 dark:bg-purple-700 text-white'>
                    {skills}
                  </span>
                })}
              </div>
